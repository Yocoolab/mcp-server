...
// Routes argv[2]: "setup"/"init" → setup wizard, "help" → help text,
...
Usage:
  ...
  yocoolab-mcp setup     Interactive setup — auto-detects agents (Claude Code, Roo, Cline, Cursor, Windsurf)
...
Environment variables (when running the server):
  YOCOOLAB_TOKEN              optional — your Yocoolab JWT (enables thread feedback tools)
  YOCOOLAB_API_URL            default https://app.yocoolab.com
  GITHUB_TOKEN                optional — enables PR-creation tools
  YOCOOLAB_BRIDGE_PORT        default 9800
  YOCOOLAB_BRIDGE_WORKSPACE   default current working directory
  YOCOOLAB_AGENT_NAME         default Claude Code
  YOCOOLAB_AGENT_TYPE         default claude-code
`);
  process.exit(0);
} else {
  startServer();
...
function startServer(): void {
  ...
  const YOCOOLAB_AGENT_TYPE = process.env.YOCOOLAB_AGENT_TYPE || 'claude-code';
  
  if (!YOCOOLAB_TOKEN) {
    process.stderr.write('[yocoolab] Warning: YOCOOLAB_TOKEN not set — thread feedback tools will be disabled.\n');
    process.stderr.write('[yocoolab] Bridge, companion chat, and activity monitor tools will still work.\n');
    process.stderr.write('[yocoolab] Get a token at https://yocoolab.com/settings → API Tokens\n');
  }
  
  // Initialize clients (api is null when no token provided)
  const api = YOCOOLAB_TOKEN ? new YocoolabApiClient(YOCOOLAB_API_URL, YOCOOLAB_TOKEN) : null;
  ...
  function registerAgent() {
    const payload = JSON.stringify({
      id: agentId,
      name: YOCOOLAB_AGENT_NAME,
      type: YOCOOLAB_AGENT_TYPE,
      cwd: YOCOOLAB_BRIDGE_WORKSPACE,
    });
    const req = http.request({
      hostname: '127.0.0.1',
      port: YOCOOLAB_BRIDGE_PORT,
      path: '/agents/register',
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'Content-Length': Buffer.byteLength(payload) },
    }, () => {});
    req.on('error', () => {}); // ignore — bridge may not be ready yet
    req.write(payload);
    req.end();
  }
  
  // Register immediately and heartbeat every 15s
  setTimeout(registerAgent, 500);
  const heartbeatInterval = setInterval(registerAgent, 15_000);
  ...
  /** Extract base64 image from a data URL and push it to content array */
  function pushScreenshot(content: ToolContent, dataUrl: string, label?: string): void {
    const match = dataUrl.match(/^data:image\/(png|jpeg|jpg|gif|webp);base64,(.+)$/);
    if (match) {
      if (label) content.push({ type: 'text' as const, text: label });
  ...
  function drainCompanionQueue(): ToolContent | null {
    ...
    for (const msg of msgs) {
      ...
      // Multi-context: show all gathered elements
      const allElements = msg.elements && msg.elements.length > 0
        ? msg.elements
        : msg.elementContext ? [msg.elementContext] : [];
      for (let i = 0; i < allElements.length; i++) {
        const ec = allElements[i] as Record<string, unknown>;
        const prefix = allElements.length > 1 ? `    Element ${i + 1}:` : '    Element:';
        if (ec.selector) lines.push(`${prefix} ${ec.selector}`);
        if (ec.tag) lines.push(`    Tag: <${ec.tag}>`);
        if (ec.text) lines.push(`    Text: "${String(ec.text).slice(0, 100)}"`);
        if (ec.styles) lines.push(`    Styles: ${JSON.stringify(ec.styles)}`);
      }
  
      // Count attachments
      const screenshotCount = msg.screenshots?.length || (msg.screenshot ? 1 : 0);
      if (screenshotCount > 0) lines.push(`    Screenshots: ${screenshotCount} attached`);
    ...
    // Attach all screenshots from all messages
    for (const msg of msgs) {
      const allScreenshots = msg.screenshots && msg.screenshots.length > 0
        ? msg.screenshots
        : msg.screenshot ? [msg.screenshot] : [];
      for (let i = 0; i < allScreenshots.length; i++) {
        const label = allScreenshots.length > 1 ? `Screenshot ${i + 1}/${allScreenshots.length}:` : undefined;
        pushScreenshot(content, allScreenshots[i], label);
      }
    }
  ...
  // ─── Yocoolab feedback tools (require YOCOOLAB_TOKEN) ─────────────────
  ...
  server.tool(
    ...
    async () => {
      ...
      for (const msg of msgs) {
        ...
        for (let i = 0; i < allElements.length; i++) {
          const ec = allElements[i] as Record<string, unknown>;
          const heading = allElements.length > 1 ? `### Element ${i + 1}` : '### Element';
          parts.push(heading);
          parts.push(`**Selector:** \`${ec.selector || ec.tag || 'unknown'}\``);
          if (ec.tag) parts.push(`**Tag:** \`<${ec.tag}>\``);
        ...
        const screenshotCount = msg.screenshots?.length || (msg.screenshot ? 1 : 0);
        if (screenshotCount > 0) parts.push(`*${screenshotCount} screenshot(s) attached below*`);
        parts.push('---', '');
      }
  ...
  // ─── AI Assistant tools (require YOCOOLAB_TOKEN) ────────────────────────
